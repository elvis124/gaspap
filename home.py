...
class Homelogin(Screen):
    ...
    def password(self, *args):
        self.grid = GridLayout(size_hint=(1, 1), background_color=[0,1,0,1],cols=1)
        self.text = TextInput(hint_text='email address')
        ...
        self.p = Popup(title='password Recovery', title_align='center', title_color=[0,1,0,1], auto_dismiss=False, content=self.grid, size_hint=(.5, .2))
        self.p.open()
    def n_password(self, *args):
        self.p.dismiss()
        d = 'Goodperwww'
        self.content = Label(text='your forgotten password:  %s' %d )
...
class ShopManager(Screen):
    ...
    def orders(self, *args):
        ...
        self.label = Button(text='latested orders',on_press=self.previous_orders,size_hint=(.1, .1), color=[0,1,0,1])
        self.ids['orderbuttons'].add_widget(self.label)
        self.button = Button(text='previous orders',on_press=self.latested_orders,size_hint=(.1, .1) ,color=[0,1,0,1])
        self.ids['orderbuttons'].add_widget(self.button)
    def stock(self, *args):
        self.ids['orderbuttons'].clear_widgets()
        self.ids['orderbuttons'].clear_widgets()
        self.ids['screenlayout'].clear_widgets()
        ...
        self.but = Button(text='last updates', size_hint=(.1, .1),on_press=self.last_update,color=[0,1,0,1])
        self.ids['orderbuttons'].add_widget(self.but)
        self.butt = Button(text='preview', size_hint=(.1, .1),on_press=self.preview,color=[0,1,0,1])
        self.ids['orderbuttons'].add_widget(self.butt)
    def last_update(self, *args):
        self.ids['screenlayout'].clear_widgets()
        for i in range(7):
            self.but = Button(text='phonenomen', size_hint=(1, .2))
            self.ids['screenlayout'].add_widget(self.but)

    def add_stock(self, *args):
        self.ids['screenlayout'].clear_widgets()
        for i in range(7):
            self.but = Button(text='hallow', size_hint=(1, .2))
            self.ids['screenlayout'].add_widget(self.but)

    def preview(self, *args):
        self.ids['screenlayout'].clear_widgets()
        for i in range(7):
            self.but = Button(text='green', size_hint=(1, .2))
            self.ids['screenlayout'].add_widget(self.but)

    def previous_orders(self, *args):
        self.ids['screenlayout'].clear_widgets()
        for i in range(7):
            self.but = Button(text='world', size_hint=(1, .2))
            self.ids['screenlayout'].add_widget(self.but)
    def latested_orders(self, *args):
        self.ids['screenlayout'].clear_widgets()
        for i in range(7):
